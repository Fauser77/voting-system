// Pode rodar no console do hardhat ou criar um script rápido
const { ethers } = require("hardhat");

async function checkBlock() {
    const provider = ethers.provider;
    const block = await provider.getBlock(309);
    
    console.log("=== BLOCO 309 ===");
...
const CONFIG = {
    ...
    NUM_VOTERS: 1000,        // Quantidade de eleitores a registrar e votar
...
async function registerVoters(contract, adminWallet, voters) {
    console.log(`\n📝 Registrando ${voters.length} eleitores...`);
    ...
    // Registrar sequencialmente para evitar conflito de nonce
    for (let i = 0; i < voters.length; i++) {
        const voter = voters[i];
        const cpfHash = hre.ethers.keccak256(hre.ethers.toUtf8Bytes(`cpf-benchmark-${i}`));
        
        const tx = await contract.connect(adminWallet).registerVoterWithCPF(voter.address, cpfHash);
        const receipt = await tx.wait();
        gasUsedList.push(receipt.gasUsed);
        
        if ((i + 1) % 10 === 0) {
            process.stdout.write(`  Registrados: ${i + 1}/${voters.length}\r`);
        }
    }
    
    const endTime = Date.now();
    console.log(`\n✅ Registro concluído em ${((endTime - startTime) / 1000).toFixed(2)}s`);
    ...
    return {
        ...
        gasUsed: gasUsedList,
        avgGas: gasUsedList.reduce((a, b) => a + BigInt(b), 0n) / BigInt(gasUsedList.length)
    };
}
...
async function main() {
    ...
    const registrationResults = await registerVoters(contract, adminWallet, voters);
